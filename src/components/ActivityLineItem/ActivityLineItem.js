...
class ActivityLineItem extends React.Component {
  ...
  render() { 
    ...
    <div className="ActivityLineItem col-md-6 col-md-offset-3">
       <img className="lineItemImage" src={activity.imgUrl} alt={activity.imgUrl} />
      <h1 className="activityName">{activity.name}</h1>
      <div className="lineItemDescription">
      <p className="time"><span className="tag">{Time}</span> - {activity.time}</p>
          <p className="address"><span className="tag">{Address}</span> - {activity.address}</p>
          <p className="type"> <span className="tag">{ActivityType}</span> - {activity.type}</p>
          <p className="theme"><span className="tag">{Theme}</span> - {activity.theme}</p>
          <p className="description"><span className="tag">{What}</span> - {activity.description}</p>
          <a href={url} >{url}</a>
      </div>
    </div>
   );
